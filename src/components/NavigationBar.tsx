...
import { Sparkles, LayoutDashboard, Calendar, Briefcase, LogOut, Menu, X } from 'lucide-react';
...
interface NavigationBarProps {
    /** Custom navigation links - if not provided, uses default worker navigation */
    navigationLinks?: NavigationLink[];
    /** User display name - defaults to auth context user name */
    userName?: string;
    /** User role display - defaults to auth context user role */
    userRole?: string;
}

const defaultNavigation: NavigationLink[] = [
    { name: 'Dashboard', href: '/worker', icon: <LayoutDashboard className="h-4 w-4" /> },
    { name: 'My Bookings', href: '/worker/bookings', icon: <Calendar className="h-4 w-4" /> },
    { name: 'My Services', href: '/worker/services', icon: <Briefcase className="h-4 w-4" /> },
];

export default function NavigationBar({
    navigationLinks = defaultNavigation,
    userName,
    userRole
}: NavigationBarProps) {
    ...
    const displayName = userName || user?.full_name || user?.email?.split('@')[0] || 'User';
